...
$Current\rightarrow Action\rightarrow Future\rightarrow Reward\rightarrow Refine$
...
| 模块                     | 作用                | 输入                                     | 输出                          |
| ---------------------- | ----------------- | -------------------------------------- | --------------------------- |
| VLM / Shared Latent    | 多模态信息融合           | 图像$I_t$、BEV$B_t$、历史动作$A_{t-1}$、文本$T_t$ | 潜状态 $H_t$                   |
...
$v_k=DiT_\theta(B'_t,A_{future},x_k,k/N)$
...
3. 加入 feature-level supervision 后 PDMS 从 **87.9→91.3**，说明共享 latent 不能只靠最终任务监督，需要显式特征对齐
   ...
   特征监督：利用真实未来观测经过冻结的 VLM 和 Cross-Attention 得到 GT future BEV latent $\mathcal{B}'_{t+\Delta t}$，再通过 Flow Matching loss $\mathcal{L}_{FM}$ 监督 action-conditioned DiT，使模型学习动作条件下的未来特征演化![[Pasted image 20260911180657.png]]
...
**论文的创新不是“VLA+WM”，而是让 WM 学会 $Action\rightarrow Future$，再让预测出的 Future 反过来优化 Action。